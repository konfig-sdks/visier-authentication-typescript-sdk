...
import { readableStreamToString, VisierAuthenticationError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new VisierAuthenticationError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new VisierAuthenticationError(e, e.response?.data)
                }
                if (innerError instanceof VisierAuthenticationError) {
                    // Got "VisierAuthenticationError" from the above try block
                    throw innerError;
                }
